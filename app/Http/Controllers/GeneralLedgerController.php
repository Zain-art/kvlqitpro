...
class GeneralLedgerController extends Controller
{
    public function ledger($general_ledger_account_id)
    {
      /***
        ...
        return view('ledger/list', array('data' => $data, 'ledgerId' => $general_ledger_account_id, 'account' => $ledger_data));
    }
...
/***
        ...
        $ledger_data = [];
            if($account->account_type_id == 1){
                $ledger_data = DB::table('customers')->where('general_ledger_account_id',$request->general_ledger_account_id)->first();
            }
            if($account->account_type_id == 2){
                $ledger_data = DB::table('vendors')->where('general_ledger_account_id',$request->general_ledger_account_id)->first();
            }
            if($account->account_type_id == 3){
                $ledger_data = DB::table('employee')->where('general_ledger_account_id',$request->general_ledger_account_id)->first();
            }
        return view('ledger/list', array('data' => $data, 'from_date' => $request->from_date, 'to_date' => $request->to_date,'account' => $ledger_data));
    }
    ...
    public function ledgerAccountsList()
    {
        ...
        $list = DB::table('general_ledger_accounts')
            ...
            ->paginate(20);
        return view('ledgerAccounts/list', array('ledgerAccountsList' => $list));
    }
    ...
    public function saveLedgerAccount(Request $request)
    {
        ...
        $validator = Validator::make(
            $request->all(),
            [
                'name' => 'required|min:3|max:20|unique:general_ledger_accounts',
    ...
    public function searchGeneralAccounts(Request $request)
    {
        $list = DB::table('general_ledger_accounts')
            ...
            ->orderByDesc('general_ledger_accounts.id')
            ->paginate(20);
        return view('ledgerAccounts/list', array('ledgerAccountsList' => $list, 'searchQuery' => $request->ledger_name));
    }
    ...
    public function accountReceivable()
    {
        ...
        if (!empty($listAccounts)) {
            foreach ($listAccounts as $account) {
                ...
                $account->balance = $this->getAccountBalance($account->id, $journal_sum_rule, '', Carbon::now()->format('Y-m-d'), '');
                $chart_of_account = DB::table('chart_of_accounts')
