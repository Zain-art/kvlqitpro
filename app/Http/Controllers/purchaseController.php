...
class purchaseController extends Controller
{
    ...
    public function purchasewiseitemlist()
    {
        $list = DB::table('purchases')->join('vendors', 'purchases.vendor_id', '=', 'vendors.id')
            ->where('purchases.branch', Auth::user()->branch)
            ->orderByDesc('purchases.id')
            ->paginate(20);
        $items = DB::table('items')->get();
        $net_total = DB::table('purchases')->sum('net_total');
    ...
    public function savePurchase(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return response()->json(['success' => true, 'message' => 'Purchase Invoice added successfully..', 'redirectUrl' => '/purchases/purchaselist'], 200);
        }
    }
    ...
    public function searchPurchaseitemwise(Request $request)
    {
        $Queries = array();
        if (empty($request->from_date) && empty($request->to_date) && empty($request->vendor_id) && empty($request->invoice_number)) {
            return redirect('purchases/purchaselist');
        ...
        $vendors = DB::table('vendors')->get();
        $items = DB::table('items')->get();
        return view('purchases.purchasewiseitemlist', array('purchaselist' => $result, 'from_date' => $request->from_date, 'to_date' => $request->to_date, 'vendor_id' => $request->vendor_id, 'invoice_number' => $request->invoice_number, 'queries' => $Queries, 'net_total' => $net_total, 'net_qty' => $net_qty,'items'=>$items, 'net_pcs' => $net_pcs, 'vendors' => $vendors));
    }
    ...
    public function purchasePdfitemwise($from_date, $to_date, $vendor_id, $invoice_number)
    {
        $query = DB::table('purchases')->join('vendors', 'purchases.vendor_id', '=', 'vendors.id')
            ->select('purchases.*', 'vendors.name as vendor_name')->where('purchases.branch', Auth::user()->branch);
        if ($from_date != 'none' && $to_date != 'none') {
        ...
        $pdf = PDF::loadView('purchases.purchasewiseitemlistpdf', $data);
        return $pdf->stream('pagePdf.pdf');
    }
