    ...
    <div class="app-main__outer">
        <div class="app-main__inner">
            ...
            <div class="text-right">
                <a href="{{route('salePdfitemwise',['from_date'=>(isset($from_date)) ? $from_date : 'none','to_date'=>(isset($to_date)) ? $to_date : 'none','vendor_id'=>(isset($vendor_id)) ? $vendor_id : 'none','invoice_number'=>(isset($invoice_number)) ? $invoice_number : 'none' ])}}" target="_blank" class="btn btn-outline-success mb-2 pdf">Download PDF</a>

            </div>
            <div class="row card mx-0 mb-2 pt-1">
                <div class="col-md-12">
                    <form action="{{route('searchSaleitemwise')}}" method="post">
                        @csrf
                        ...
                        <div class="row no-gutters">
                            ...
                            {{-- <div class="form-group pt-1">
                                ...
                                <select class="js-example-basic-single form-control" placeholder="Select Customer" name="agent_id" id="customer_id">
                                    <option value="">Select Agent</option>
                                    @if(!empty($agents))
                                    @foreach($agents as $vendor)
                                    <option value="{{$vendor->id}}" {{(isset($agent_id)) ? ($vendor->id==$agents_id) ? 'Selected' : '' : ''}}> {{$vendor->Name}} </option>
                                    @endforeach
                                    @endif
                                </select>
                            </div> --}}
                    ...
                    <div class="main-card mb-3 card">
                        ...
                        <div class="table-responsive">
                            <table class="align-middle mb-0 table table-borderless table-striped table-hover">
                                <thead>
                                    <tr>
                                        <th class="text-center">Most Item Sale</th>
                                ...
                                <tbody>
                                    @if(!empty($salewiseitemlist))
                                    @foreach($salewiseitemlist as $salelist)
                                    @php
                                    $i=1;
                                    @endphp
                                    @if(!empty(unserialize($salelist->items_detail)))
                                    @php
                                    $i=1;
                                    @endphp
                                    @foreach(unserialize($salelist->items_detail) as $invoiceItem)
                                    <tr>
                                        <td class="text-center text-muted">{{$i}}</td>
                                        <td class="text-center text-muted">{{$salelist->invoice_number}}</td>
                                        {{-- <td class="text-center">{{$salelist->Name}}</td> --}}
                                        <td class="text-center"> @if(!empty($items))
                                            @foreach($items as $item)
                                            {{(isset($salelist)) ? ($item->id == $invoiceItem['item_id']) ? $item->name : '' : ''}}
                                            @endforeach
                                            @endif
                                        </td>
                                        <td class="text-center">{{(isset($salelist)) ? $invoiceItem['item_qty'] : ''}} </td>
                                        <td class="text-center">{{(isset($salelist)) ? $invoiceItem['item_price'] : ''}} </td>
                                        <td class="text-center">{{(isset($salelist)) ? $invoiceItem['amount'] : ''}}</td>
                                        <td class="text-center">{{$salelist->invoice_date}}</td>
                        ...
                        <div class="d-flex justify-content-end mr-3 card-footer">
                            ...
                            {{ $salewiseitemlist->appends(['queries'=>$queries])->links() }}
                            @elseif(isset($_GET['queries']))
                            {{ $salewiseitemlist->appends(['queries'=>$_GET['queries']])->links() }}
                            @else
                            {{$salewiseitemlist->links()}}
                            @endif
                        </div>
                    </div>
                </div>
